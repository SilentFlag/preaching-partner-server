...
pub async fn logout(refresh_token: [u8; 32], db: &sqlx::Pool<sqlx::Sqlite>) -> Result<bool,bool> {
    ...
    let insert_token_query = sqlx::query("DELETE FROM tokens WHERE token = ?")
        .bind(hex::encode(&refresh_token));
    let query_result = insert_token_query.execute(db).await;

    if let Err(result) = query_result {
        println!("Something went wrong logging out refresh token from database, rows affected: {:?}", result);
        return Err(false);
    }

    // remove access tokens
    let insert_token_query = sqlx::query("DELETE FROM tokens WHERE user = ? AND refresh = false")
        .bind(&user_id);
    let query_result = insert_token_query.execute(db).await;

    if let Err(result) = query_result {
        println!("Something went wrong loggin out access tokens from database, rows affected: {:?}", result);
        return Err(false);
    }
...
/// TODO: handle the error of the token failing to be entered into the database
pub async fn roll_access_token(refresh_token: [u8; 32], db: &sqlx::Pool<sqlx::Sqlite>) -> Option<[u8; 32]> {
    let mut rng = SysRng;
    let mut buf = [0u8; 32];
    let _ = rng.try_fill_bytes(&mut buf); // TODO: handle fail of filling bytes
    ...
    if let Err(result) = query_result {
        // TODO: handle this error
        println!("Something went wrong inserting access token into database, error: {:?}", result)
    }
...
pub async fn revoke_tokens(user: u32, db: &sqlx::Pool<sqlx::Sqlite>) -> Result<bool, bool> {
    ...
    if let Err(result) = query_result {
        println!("Something went wrong revoking tokens from database, rows affected: {:?}", result);
        return Err(false);
    }
    Ok(true)
}